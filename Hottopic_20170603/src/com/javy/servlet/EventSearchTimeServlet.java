...
import com.javy.handler.BayesianHandler;
...
/**
 * 有时间比较的热点事件查询 Servlet implementation class EventSearchTimeServlet
 */
@WebServlet(name = "/EventSearchTimeServlet", urlPatterns = "/eventsearchtime.do")
public class EventSearchTimeServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public EventSearchTimeServlet() {
		super();
	...
	protected void doPost(HttpServletRequest request,
		...
		// 今日数据存储
		HashMap<String, Integer> today = null;
		// 工单列表
		...
		BayesianHandler bayesianhandler = new BayesianHandler();
		...
		try {
			if (hotpick.equals("0")) {
				...
				date.setTime(sdf.parse(t1));
				Calendar yesdate = (Calendar) date.clone();
				yesdate.add(Calendar.DATE, -1);

				// 得到两个map
				yesterday = databasehandler.getfreitems(yesdate, yesdate);
				today = databasehandler.getfreitems(date, date);
				// 得到原文本
				texts = databasehandler.gettexts(date, date);
			} else if (hotpick.equals("1")) {
				String t2 = request.getParameter("t2");
				String[] date = t2.split(":");
				Calendar date1 = Calendar.getInstance();
				date1.setTime(sdf.parse(date[0]));
				Calendar date2 = Calendar.getInstance();
				date2.setTime(sdf.parse(date[1]));

				Calendar yesdate1 = (Calendar) date1.clone();
				yesdate1.add(Calendar.DATE, -7);
				Calendar yesdate2 = (Calendar) date2.clone();
				yesdate2.add(Calendar.DATE, -7);

				// 得到两个map
				yesterday = databasehandler.getfreitems(yesdate1, yesdate2);
				today = databasehandler.getfreitems(date1, date2);
				// 得到原文本
				texts = databasehandler.gettexts(date1, date2);
			} else if (hotpick.equals("2")) {
				...
				switch (t3) {
				case "1": {
					date1.setTime(sdf.parse("2017-02-01"));
					date2.setTime(sdf.parse("2017-02-28"));
					yesdate1.setTime(sdf.parse("2017-01-01"));
					yesdate2.setTime(sdf.parse("2017-01-31"));
				}
					break;
				}
				...
				// 得到原文本
				texts = databasehandler.gettexts(date1, date2);
			} else if (hotpick.equals("3")) {
				// 对比时间
				String t41 = request.getParameter("t41");
				// 查询时间
				String t42 = request.getParameter("t42");
				Calendar date = Calendar.getInstance();
				date.setTime(sdf.parse(t42));
				Calendar yesdate = Calendar.getInstance();
				yesdate.setTime(sdf.parse(t41));
				...
				// 得到原文本
				texts = databasehandler.gettexts(date, date);
			} else if (hotpick.equals("4")) {
				// 对比时间
				String t51 = request.getParameter("t51");
				String t52 = request.getParameter("t52");
				// 查询时间
				String t53 = request.getParameter("t53");
				Calendar date = Calendar.getInstance();
				date.setTime(sdf.parse(t53));
				Calendar yesdate1 = Calendar.getInstance();
				...
				// 得到原文本
				texts = databasehandler.gettexts(date, date);
			} else if (hotpick.equals("5")) {
				// 对比时间
				String t61 = request.getParameter("t61");
				String t62 = request.getParameter("t62");
				// 查询时间
				String t63 = request.getParameter("t63");
				String t64 = request.getParameter("t64");
				Calendar date1 = Calendar.getInstance();
				date1.setTime(sdf.parse(t63));
				Calendar date2 = Calendar.getInstance();
				date2.setTime(sdf.parse(t64));
				Calendar yesdate1 = Calendar.getInstance();
				yesdate1.setTime(sdf.parse(t61));
				Calendar yesdate2 = Calendar.getInstance();
				yesdate2.setTime(sdf.parse(t62));
			...
			// ---------------------------------------------同义词库获取-----------------------------------------//
			// 合并两个hashmap
			HashMap<String, Integer> merge = hashhandler.mergeHashMap(
					yesterday, today);
			// 今日占比
			HashMap<String, Double> todaypercent = hashhandler.getTodayPercent(
					today, merge);
			double mergeavg = bayesianhandler.getIntAverage(merge);
			double percentavg = bayesianhandler.getDoubleAverage(todaypercent);
			// 贝叶斯平均值计算
			HashMap<String, Double> todaybayesian = bayesianhandler
					.getTodayBayesian(todaypercent, merge, mergeavg, percentavg);
			// 贝叶斯平均值排序
			List<Map.Entry<String, Double>> sortedbayesian = hashhandler
					.SortHash(todaybayesian);
			// -----------------------------------------贝叶斯平均计算-----------------------------------------//
